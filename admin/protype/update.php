...
require '../../models/protype.php';
    $id = $_GET['id'];
    $Protype = new Protype();
    $type = $Protype->getProtypeById($id);
    foreach ($type as $key_type => $value) {
        $type_name = $value['type_name'];
    }  
if(isset($_POST['type_name'])){
        $type_name1 = $_POST['type_name'];
        $Protype = new Protype();
        $type1 = $Protype->getProtypeByName($type_name1);
        $num = count($type1);
        if($num > 0){
            $error =  "Tên Danh Mục Đã Tồn Tại";
        }else{
        $sql = "UPDATE protypes SET type_name ='$type_name1' WHERE type_id ='$id'";
        $db = new Db();
        $db->SIUD($sql);
                ...
                <div class="span12">
                    <div class="widget-box">
                        ...
                        <form action="" method="POST">
                            <label>Tên Protypes</label>
                            <input required value="<?php if(isset($type_name)) echo $type_name ?>" required class="form-control" type="text" name="type_name"><br>
                            
                            <div class="error">
                            <?php if(isset($error)) echo $error ?><br>
                            </div>
                            <input class="btn btn-info" type="submit" value="Edit Protypes">
                        </form>
                    </div>
                </div>
            </div>
